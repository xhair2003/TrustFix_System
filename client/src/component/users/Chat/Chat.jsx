...
const Chat = () => {
    ...
    const [messageInput, setMessageInput] = useState('');
    const [chatList, setChatList] = useState([]); // Danh sách các cuộc trò chuyện
    const [hasNewMessages, setHasNewMessages] = useState({}); // Theo dõi tin nhắn mới cho ChatList
    const [hasNewMessageForIcon, setHasNewMessageForIcon] = useState(false); // Theo dõi tin nhắn mới cho icon Chat
    ...
    useEffect(() => {
        const handleReceiveMessage = (message) => {
            if (message.senderId !== user_id && message.receiverId === user_id) {
                ...
                setHasNewMessageForIcon(true); // Hiển thị dấu chấm đỏ trên icon Chat
                // Chỉ gọi getChatHistory nếu chat đang mở hoặc liên quan đến selectedChat
                if (!selectedChat || selectedChat.id === message.senderId) {
                    if (!isFetchingRef.current) {
                        isFetchingRef.current = true;
    ...
    const toggleChatDropdown = () => {
        setIsChatOpen(!isChatOpen);
        setHasNewMessageForIcon(false); // Xóa dấu chấm đỏ khi click vào icon Chat
    };
    ...
    return (
        <div className="chat-container">
            <FaComments
                className={`chat-icon ${hasNewMessageForIcon ? 'has-new-message' : ''}`}
                onClick={toggleChatDropdown}
            />
            {isChatOpen && (
                <div className="user-chat-popup">
