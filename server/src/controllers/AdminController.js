const { ServiceIndustry, Service, Complaint, User, Request, Transaction, Wallet, Role, Rating, RepairmanUpgradeRequest, Vip , Guide } = require("../models");
...
//     try {
//       console.log("Bắt đầu kiểm tra các gói VIP đã hết hạn...");
  
//       // Lấy danh sách các yêu cầu nâng cấp có gói VIP đã hết hạn
//       const expiredVips = await RepairmanUpgradeRequest.find({
//         vip_time_expire: { $lte: new Date() }, // Ngày hết hạn nhỏ hơn hoặc bằng ngày hiện tại
//         vip_id: { $ne: null }, // Chỉ kiểm tra các yêu cầu có gói VIP
//       }).populate("user_id", "email firstName lastName");
  
//       if (expiredVips.length === 0) {
//         console.log("Không có gói VIP nào đã hết hạn.");
//         return;
//       }
  
//       // Xử lý từng yêu cầu nâng cấp có gói VIP hết hạn
//       for (const vip of expiredVips) {
//         // Gửi email thông báo
//         const user = vip.user_id;
//         if (user && user.email) {
//           const emailContent = `
//             <p>Chào ${user.firstName} ${user.lastName},</p>
//             <p>Gói VIP của bạn đã hết hạn vào ngày <strong>${vip.vip_time_expire.toLocaleDateString()}</strong>.</p>
//             <p>Vui lòng gia hạn gói VIP để tiếp tục sử dụng các dịch vụ ưu đãi.</p>
//             <p>Cảm ơn bạn đã sử dụng dịch vụ của chúng tôi!</p>
//           `;
  
//           await sendEmail(user.email, "Thông báo: Gói VIP đã hết hạn", emailContent);
//           console.log(`Đã gửi email thông báo hết hạn gói VIP cho ${user.email}`);
//         }
  
//         // Cập nhật trạng thái gói VIP
//         vip.vip_id = null; // Xóa gói VIP
//         vip.vip_time_expire = null; // Xóa ngày hết hạn
//         await vip.save();
//       }
  
//       console.log(`Đã xử lý ${expiredVips.length} gói VIP hết hạn.`);
//     } catch (error) {
//       console.error("Lỗi khi kiểm tra gói VIP đã hết hạn:", error);
//     }
//   };
  
//   // Lên lịch chạy cron job mỗi ngày lúc 00:00
//   cron.schedule("0 0 * * *", async () => {
//     console.log("Chạy cron job kiểm tra gói VIP...");
